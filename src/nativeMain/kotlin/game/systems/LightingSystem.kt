...
class LightingSystem(engine: Engine) : ISystem(engine) {
    ...
    override fun update(entity: Entity) {
        ...
        if (brezenham(playerTransform.position, lighting.renderPosition) { x, y -> walls.findWallAtPoint(Vector2(x, y)) }) {
...
//        val player = engine.entityManager.getByTag("player")?.first()
//        val playerTransform = player?.getComponent<Transform>()!!
    ...
    private fun isVisibleWall(position: Vector2): Boolean {
        repeat(3) { i ->
            repeat(3) { j ->
                if (i != 0 && j != 0)
                    if (pick(position.x + i - 1, position.y + j - 1)) return false
            }
        }
        return true
    }
    ...
    private fun pick(x: Int, y: Int): Boolean {
        ...
        return points[Vector2(x, y)]?.getComponent<Lighting>()?.lighted!! && Terminal.pick(Vector2(x, y), 4) != '#'.toInt()
    }
}
